...
int main () {
    ...
    for (int x = 0; x < 2; x++) {
        for (int y = 0; y < size_arr; y++) {
            scanf ("%d", &array [x][y]);
            if (x == 0) {if (array [x][y] > largest1) {largest1 = array [x][y];}}
            else {if (array [x][y] > largest2) {largest2 = array [x][y];}}
        } printf ("\n");
    ...
    for (int c = 0; c < largest1 + largest2; c++) {preset_val [c] = 0;}
    for (int z = 0; z < iteration; z++) {
        if (z == 0) {
            ...
            for (int f = 0; f < size_arr; f++) {
                ...
                printf ("%d %d %d\n", arrc [f], new_array [0][f], new_array [1][f]);
                preset_val [arrc [f] - 1] += 1;
            } int count_distinct = 0;
            for (int g = 0; g < largest1 + largest2; g++) {
                if (preset_val [g] != 0) {count_distinct++;}
                preset_val [g] = 0;
            } if (count_distinct >= 3) {
                printf ("YES\n");
                exit (0);
            ...
            for (int d = 0; d < largest1 + largest2; d++) {
                ...
                else {if (preset_val [d] != 0) {count_distinct++;}}
            } if (count_distinct >= 3) {
                printf ("YES\n");
                exit (0);
            } for (int e = 0; e < largest1 + largest2; e++) {preset_val [e] = 0;}
        } printf ("\n");
    } printf ("NO\n");
    return 0;
}
